...
// Get references to the background and light effect
    ...
    // Split the heading into individual words wrapped in spans
    heading.innerHTML = heading.textContent
    .split(' ')
    .map(word => `<span class="word">${word}</span>`)
    .join(' ');

    // Get all the words as span elements
    ...
    // Loop through each word and calculate its position relative to the cursor
    words.forEach(word => {
        const wordRect = word.getBoundingClientRect();
        const wordCenterX = wordRect.left + wordRect.width / 2;
        const wordCenterY = wordRect.top + wordRect.height / 2;

        const distanceX = Math.abs(clientX - wordCenterX);
        const distanceY = Math.abs(clientY - wordCenterY);
        
        const radius = 100; // Set the radius of the effect (half of the lightEffect size)

        // Apply hover effect only to the word inside the spotlight (within the circle area)
        if (distanceX <= radius && distanceY <= radius) {
        word.classList.add('in-spotlight');
        } else {
        word.classList.remove('in-spotlight');
        }
    });
    });
    ...
    words.forEach(word => word.classList.remove('in-spotlight')); // Remove effect if mouse leaves
    });
