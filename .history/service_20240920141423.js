...
lightSection.addEventListener('mousemove', (e) => {
  ...
  // Loop through each letter and calculate its position relative to the cursor
  letters.forEach(letter => {
    const letterRect = letter.getBoundingClientRect();
    const letterCenterX = letterRect.left + letterRect.width / 2;
    const letterCenterY = letterRect.top + letterRect.height / 2;

    const distanceX = Math.abs(clientX - letterCenterX);
    const distanceY = Math.abs(clientY - letterCenterY);
    
    const radius = 100; // Set the radius of the effect (half of the lightEffect size)
letter.style.hover = '-webkit-text-stroke: 2px white;'
letter.style.hover = 'color: transpearent'
    // Apply hover effect only to the letter inside the spotlight (within the circle area)
...
lightSection.addEventListener('mouseleave', () => {
  ...
  letters.forEach(letter => letter.classList.remove('in-spotlight')); // Remove effect if mouse leaves
});
