...
const cursor = document.querySelector('.cursor-effect__cursor');

// Throttle the mouse move events using requestAnimationFrame
let isMoving = false;
...
window.addEventListener('mousemove', (e) => {
  if (!isMoving) {
    window.requestAnimationFrame(() => {
      ...
      cursor.classList.add('cursor-effect__cursor--visible');
      isMoving = false;
    });
    isMoving = true;
  }
...
// Hide the cursor when not hovering over interactive areas
window.addEventListener('mouseleave', () => {
  cursor.classList.remove('cursor-effect__cursor--visible');
});
