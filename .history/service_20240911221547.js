...
const container = document.querySelector('.my_scroll');
const containerWidth = container.scrollWidth - window.innerWidth;
const triggerElement = document.querySelector('.sec_7');

// Define the GSAP animation for horizontal scrolling
gsap.to(container, {
    x: -containerWidth,
    ease: "none",
    scrollTrigger: {
        trigger: triggerElement,
        start: "top 30%", // Start when the section hits the top
        end: () => `+=${containerWidth}`, // End after the horizontal scroll is complete
        scrub: 1, // Smooth scrubbing
        pin: true, // Keeps the section pinned while scrolling horizontally
