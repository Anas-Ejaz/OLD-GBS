...
lightSection.addEventListener('mousemove', (e) => {
  ...
  // Calculate the distance between the mouse and the heading's center
  ...
  // Check if the cursor is within the light effect area
  const distanceX = Math.abs(clientX - headingCenterX);
  const distanceY = Math.abs(clientY - headingCenterY);
  
  const radius = 100; // Set the radius of the effect (half of the lightEffect size)

  // Apply hover effect only inside the spotlight (within the circle area)
  if (distanceX <= radius && distanceY <= radius) {
    heading.classList.add('in-spotlight');
  } else {
    heading.classList.remove('in-spotlight');
  }
});
...
lightSection.addEventListener('mouseleave', () => {
  ...
  heading.classList.remove('in-spotlight'); // Remove effect if mouse leaves
});
